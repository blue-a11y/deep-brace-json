...
- **其他**：根容器使用 `px-3 py-2` / `gap-2`；面板拖拽分栏（比例持久化）、⌘/Ctrl+Enter 历史遗留已随自动解析移除、编辑器滚动修复（cm-theme 高度链）、默认浅色主题、状态栏统计；`vercel.json` 配置 `ignoreCommand`（仅 `.md` 变更时退出码 0 → Vercel 跳过构建，纯文档提交不再产生部署）
