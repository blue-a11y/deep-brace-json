...
- **其他**：根容器使用 `px-3 py-2` / `gap-2`；面板拖拽分栏（比例持久化）、⌘/Ctrl+Enter 历史遗留已随自动解析移除、编辑器滚动修复（cm-theme 高度链）、默认浅色主题、状态栏统计；~~vercel.json 配置 ignoreCommand（仅 .md 变更时跳过构建）~~ 已移除——纯文档提交会取消在途的代码构建，导致代码不部署（2026-08-27 实测踩坑）
...
_2026-08-27 更新；详细提交历史见 git log。曾用 `ignoreCommand` 跳过纯文档构建，因会取消在途代码部署已移除。_
